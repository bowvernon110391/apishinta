...
class PdfLembarHitungPIBK extends PDF_MC_Table {
    protected $pibk = null;
    ...
    public function generateFirstPage() {
        if (!$this->pibk) {
            throw new BadRequestHttpException("PIBK data not provided");
        }

        if (!$this->pibk->is_locked) {
            throw new BadRequestHttpException("CD belum ditetapkan!");
        }

        // feed data
        $data = $this->pibk->computePungutanImpor();

        $lock = $this->pibk->lock;
        $petugas = $lock->petugas;

        $payer = $this->pibk->payer;
        ...
        $pdf->font();
        $pdf->Cell(0, 4, "PIBK No. : {$nomor_lengkap_dok}        DATE: {$tgl_dok}", 0,1,'C');
