...
class PdfSSPCP extends Fpdf {
    protected $sspcp = null;

    // constructor follow usual style
    public function __construct(SSPCP $s)
    {
        $this->sspcp = $s;
    ...
    public function generateFirstpage() {
        ...
        $trfbm = '';
        $bmrup = $this->sspcp->total_bm;

        $trfdenda = '';
        $dendarup = 0;

        $trfppn = '';
        $ppnrup = $this->sspcp->total_ppn;

        $trfppnbm = '';
        $ppnbmrup = $this->sspcp->total_ppnbm;

        $trfpph = '';
        $pphrup = $this->sspcp->total_pph;

        $blnpajak = getBulan($this->sspcp->tgl_dok);
        $thnpajak = getTahun($this->sspcp->tgl_dok);
        ...
        $nomorsspcp = $this->sspcp->nomor_lengkap;
        $tanggalsspcp = formatTanggal($this->sspcp->tgl_dok);

        $namahgr = $this->sspcp->nama_pejabat;
        $niphgr = $this->sspcp->nip_pejabat;
